...
!!! info "Architecture diagram tools"
    ...
    The diagram is exported as an SVG image from draw.io and will be maintained as the system evolves. Click it to see it in all of its glory.
...
| **companion**<br>`services/companion`               | Converts control commands to MAVLink messages and publishes telemetry back over the same WebSocket connection                                                                                 |
| **video**<br>`services/companion/deploy`            | MediaMTX container streaming live video from the companion camera                                                                                                                                                    |
| **Kill switch**<br>`embedded/esp32/kill_switch.ino` | ESP32 running a C++ Arduino sketch which, upon receiving a kill switch signal on the mapped receiver channel, shuts down the companion computer — crucial for safe operations with an experimental system |
